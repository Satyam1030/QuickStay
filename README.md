...
### 👤 Guest Experience
- 🔍 **Interactive Search**: Search for accommodations by location, dates, and guest capacity.
- 🌟 **Featured Destinations & Exclusive Offers**: Hand-picked travel spots and curated promotional stay offers.
- 🛏️ **Room Listings & Details**: View complete room information including descriptions, pricing, amenities, and image galleries.
- 📅 **Booking Management**: Track active reservations and view booking history in real-time.
- 🔐 **Authentication**: Fast and secure login/signup powered by Clerk.

### 🏢 Hotel Owner / Admin Panel
- 📊 **Dashboard Overview**: Monitor total bookings, revenue, and active listings.
- ➕ **Add Room Listings**: Interface to create room listings with rich descriptions, pricing, amenity tags, and multi-image uploads via Cloudinary.
- 🛠️ **Manage Listings**: Easily toggle availability, edit, or delete existing property listings.

### ⚡ Clerk Webhooks Integration
- 🔄 **Real-Time User Synchronization**: Automatically sync user account creation, updates, and deletions from Clerk to MongoDB using Svix webhook verification (`/api/clerk`).
...
### Deployment & Hosting
- **Vercel**: Native Vercel deployment configuration (`vercel.json`) for SPA routing and serverless backend API functions.
...
│   │   └── pages/               # Guest & Owner dashboard pages
...
└── server/                      # Node.js + Express Backend
    ...
    ├── models/                  # Mongoose data schemas (User, etc.)
...
2. Add an Endpoint pointing to your deployed backend or tunnel URL: `https://your-domain.com/api/clerk`.
...
Both `client` and `server` folders contain custom `vercel.json` files designed for smooth deployment on Vercel:

- **Frontend Deployment**: Import the repository on Vercel, select `client` as the Root Directory, and configure environment variables.
- **Backend Deployment**: Import the repository on Vercel, select `server` as the Root Directory, and set all server environment variables in Vercel settings.
